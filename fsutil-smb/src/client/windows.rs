...
use fsutil_core::fs::{File, Metadata, ReadStream, UnixPex, Welcome, WriteStream};
use fsutil_core::{RemoteError, RemoteErrorType, RemoteFileSystem, RemoteResult};
...
impl SmbFileSystem {
    /// Instantiates a new SmbFileSystem
...
mod test {
    ...
    fn test_should_be_sync() {
        let client = SmbFileSystem::new(SmbCredentials::new("pippo", "pippo"));
    ...
    #[cfg(feature = "with-containers")]
    fn init_client() -> SmbFileSystem {
        let mut client = SmbFileSystem::new(
    ...
    #[cfg(feature = "with-containers")]
    fn finalize_client(mut client: SmbFileSystem) {
