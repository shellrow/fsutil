...
impl FtpFileSystem {
    /// Instantiates a new `FtpFileSystem`
...
mod test {
    ...
    fn should_initialize_ftp_filesystem() {
        let client = FtpFileSystem::new("127.0.0.1", 21);
    ...
    fn should_connect_with_ftps() {
        let mut client = FtpFileSystem::new("test.rebex.net", 21)
            .username("demo")
            .password("password")
            .secure(false, false)
            .passive_mode();
    ...
    fn should_return_not_connected_error() {
        crate::mock::logger();
        let mut client = FtpFileSystem::new("127.0.0.1", 21);
    ...
    fn test_should_be_sync() {
        let client = FtpFileSystem::new("127.0.0.1", 10021)
            .username("test")
            .password("test");
    ...
    #[cfg(feature = "with-containers")]
    fn setup_client() -> FtpFileSystem {
        let mut client = FtpFileSystem::new("127.0.0.1", 10021)
            .username("test")
            .password("test");
        assert!(client.connect().is_ok());
        // Create wrkdir
    ...
    #[cfg(feature = "with-containers")]
    fn finalize_client(mut client: FtpFileSystem) {
