...
pub trait RemoteFileSystem {
    ...
    ///
    /// By default this method will combine [`RemoteFileSystem::remove_dir`] and [`RemoteFileSystem::remove_file`] to remove all the content.
    ...
    /// Finalize [`RemoteFileSystem::create`] and [`RemoteFileSystem::append`] methods.
    ...
    /// Finalize [`RemoteFileSystem::open`] method.
    /// This method must be implemented only if necessary; in case you don't need it, just return [`Ok`]
    ...
    /// Blocking implementation of [`RemoteFileSystem::append`]
    ...
    /// Blocking implementation of [`RemoteFileSystem::create`]
    /// This method SHOULD be implemented ONLY when streams are not supported by the current file transfer.
    /// The developer using the client should FIRST try with `create` followed by `on_written`
    /// If the function returns error of kind [`RemoteErrorType::UnsupportedFeature`], then he should call this function.
    /// In case of success, returns the amount of bytes written to the remote file
    ...
    /// Blocking implementation of [`RemoteFileSystem::open`]
    /// This method SHOULD be implemented ONLY when streams are not supported by the current file transfer.
    /// (since it would work thanks to the default implementation)
    /// The developer using the client should FIRST try with [`RemoteFileSystem::open`] followed by [`RemoteFileSystem::on_read`]
    /// If the function returns error of kind [`RemoteErrorType::UnsupportedFeature`], then he should call this function.
...
mod test {
    ...
    use super::*;
    use crate::mock::MockRemoteFileSystem;

    #[test]
    fn should_be_able_to_create_trait_object() {
        let _: Box<dyn RemoteFileSystem> = Box::new(MockRemoteFileSystem {});
    }
}
