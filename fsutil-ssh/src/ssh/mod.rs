...
impl From<SshOpts> for SftpFileSystem {
    fn from(opts: SshOpts) -> Self {
        SftpFileSystem::new(opts)
    }
}

impl From<SshOpts> for ScpFileSystem {
    fn from(opts: SshOpts) -> Self {
        ScpFileSystem::new(opts)
    }
}
...
mod test {
    ...
    fn should_build_ssh_opts() {
        let opts = SshOpts::new("localhost")
            ...
            .config_file(Path::new("/home/user0/.ssh/config"), ParseRule::STRICT)
        ...
        assert_eq!(
            ...
            Path::new("/home/user0/.ssh/config")
        );
    ...
    fn should_build_sftp_client() {
        let _: SftpFileSystem = SshOpts::new("localhost").into();
    }

    #[test]
    fn should_build_scp_client() {
        let _: ScpFileSystem = SshOpts::new("localhost").into();
    }
}
